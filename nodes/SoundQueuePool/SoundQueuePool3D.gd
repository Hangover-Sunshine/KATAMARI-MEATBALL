...
var m_audio_stream_player:Array[SoundQueue3D]

func _ready():
	var children = get_children()
	
	for ci in range(get_child_count()):
		if children[ci] is SoundQueue3D:
			m_audio_stream_player.append(children[ci])
		else:
			print("Non-SoundQueue3D child detected at index %s.", ci)
		##
	##
##
