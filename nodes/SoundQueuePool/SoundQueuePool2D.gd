...
var m_audio_stream_player:Array[SoundQueue2D]

func _ready():
	var children = get_children()
	
	for ci in range(get_child_count()):
		if children[ci] is SoundQueue2D:
			m_audio_stream_player.append(children[ci])
		else:
			print("Non-SoundQueue2D child detected at index %s.", ci)
		##
	##
##
